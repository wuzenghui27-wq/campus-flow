...
import { extractProfile, normalizeWebsite, parseApplications, parseProfile, summarize, summarizeCompanies, type Application } from '../src/model.ts';
...
test('统计投递并安全恢复损坏数据', () => {
  ...
  assert.deepEqual(summarizeCompanies([...apps,{...apps[0],id:'3',role:'后端'}]), [
    { company:'A', count:2, website:'https://a.example/', positions:[{role:'前端',location:'上海',status:'面试'},{role:'后端',location:'上海',status:'面试'}], statuses:{ 已投递:0, 笔试:0, 面试:2, 录用:0 } },
    { company:'B', count:1, website:'', positions:[{role:'产品',location:'北京',status:'录用'}], statuses:{ 已投递:0, 笔试:0, 面试:0, 录用:1 } },
  ]);

...
function layoutText(items, width) {
  ...
  }).filter(c=>c.left.length>=3 && c.right.length>=3 && c.crossing.length<=words.length*0.15 &&
    pageRows.filter(row=>row.words.some(word=>c.crossing.includes(word))).length<=pageRows.length*0.25 &&
    Math.min(...c.right.map(w=>w.x))-Math.max(...c.left.map(w=>w.x+w.width))>width*0.06 &&
    Math.min(...c.left.map(w=>w.y)) < Math.max(...c.right.map(w=>w.y)) && Math.min(...c.right.map(w=>w.y)) < Math.max(...c.left.map(w=>w.y)));
